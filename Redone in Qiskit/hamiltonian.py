...
# 
# An list of tuples representing the local hamiltonian terms  in the format:
# (active, h_alpha, imap, gmap) [alpha is the indexing variable for the list]
#   active      - A list of ints that indicates which qubits this term acts on
#   h_alpha     - a unit-length superposition of the gates acting in this term
#                 eg: if the term is H = (X+Z)/sqrt(2) on one qubit,
#                   h_alpha = [0, 1/sqrt(2), 0, 1/sqrt(2)]
#                              I     X       Y      Z  (indexes are the Pauli Strings)
#   imap,gmap   - pauli_action output for the active qubits
#
# The Hamiltonian is the sum of all the terms in the list
#
#--------------------------------------#

# Apply H to a vector psi
def Hpsi(H_,psi_):
...
# The matrix representing H (in computational basis)
def Hmat(H_):
    N  = H_[0][2].shape[1]
...
#-------------------------------------------#
# Defining the different Hamiltonian models #
#-------------------------------------------#
