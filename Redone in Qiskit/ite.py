...
def ITE_FCI(H_,db,bmax,psi0=None,omega=None):
    ...
    if(psi0 is None):
        i0          = np.argmin(np.diag(Hm)) # index of the smallest diagonal element of the Hamiltonian
        psi_FCI     = np.zeros(N,dtype=complex)
        psi_FCI[i0] = 1.0 # start state is that index's basis state
    else:
        psi_FCI     = psi0.copy()
    ...
    for ib in range(nbeta):
        ...
        print(ib*db,ea,ev)
        psi_FCI  = np.dot(np.conj(U.T),psi_FCI) # change to the eigenbasis
        psi_FCI  = zeta*psi_FCI # apply the imaginary time evolution step for an imaginary time step of db
...
# calculates: (I + (-db H)/1 + (-db H)^2/(1.2) + (-db H)^3/(1.2.3) + ... ) 
# or exp( -db H ), where db is the imaginary time step
# this is the operator mapping the imaginary time evolution
