...
# Apply the alpha_'th term in H_ to psi_
...
# Imaginary time evolution of |psi_> under the alpha_th term of H_ for 
# an imaginary time step db
# Returns the normalized state after the evolution and the original norm
...
# Apply a linear combination (weights in x_) of pauli terms to psi
...
# Calculates exp(i X), where X is the linear combination of pauli strings
...
def QITE_step(H_,psi_,db,xv,check):
    ...
    nalpha = len(H_)
    # keeps track of the norm of the imaginary time evolution
    dn_    = 1.0
    ...
    for alpha in range(nalpha):
        ...
        # delta_alpha is the delta of the wavefunction between now and evolving under the alpha term
        ...
        # Pmu_psi stores the state vectors of every possible pauli string on A
        for m in range(nact):
        ...
        # ----- set linear system 
        # Get a matrix of 2 * Re(<psi_i|psi_j>) for i,j indexing the possible pauli strings
        Amat = np.dot(np.conj(Pmu_psi),Pmu_psi.T)
        Amat = 2.0*np.real(Amat)
        # In the end Amat = 2 Re(S) from the paper

        t2  = time.time()
        # Get a vector of -2 Im(<delta_alpha | psi_i>) for each possible pauli string i
        bvec = np.dot(Pmu_psi,np.conj(delta_alpha))
        bvec = -2.0*np.imag(bvec)
        # In the end bvec = 2 Re(b) from the paper
        # So, we're optimizing the system:
        # 2 Re(S) a = 2 Re(b), where a is a real vector
        # In the paper, they did Sa = b, but since a is real, we can 
        # take the real components of this system instead
        t3  = time.time()

        # Find the least squares solution to Amat x = bvec
        if(check):
