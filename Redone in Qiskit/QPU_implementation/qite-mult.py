...
from helper import run_circuit,measure_mult,propogate_mult,estimate_assignment_probs,update_alist_mult
...
def get_expectation(alist, qbits, nbits, backend, shots=1024):
	sigma_expectation = np.zeros([4**nbits],dtype=complex)
	sigma_expectation[0] = 1
	for j in range(1,4**nbits):
...
if __name__ == '__main__':
	...
	hm_list = []
	hm_list.append([])
	hm_list[0].append([[1],[1/np.sqrt(2)],[0]])
	hm_list.append([])
	hm_list[1].append([[3],[1/np.sqrt(2)],[0]])
	delta = 0.1
