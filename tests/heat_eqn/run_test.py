...
def get_theoretical_evolution(L:float,Nx:int,dx:float,Nt:int,dt:float,
                              ...
                              alpha=1.0):
    if freq == 0:
        return np.ones((Nt+1,Nx), dtype=np.float64)
    if not homogeneous_flag:
        ...
        for i,t in enumerate(np.arange(Nt+1)*dt):
            theoretical_solution[i,:] = np.sin(freq*np.pi*x/L) * np.exp(-np.power(freq*np.pi/L,2)*t*alpha)
        return theoretical_solution
    else:
        theoretical_solution = np.ones(((Nt+1),Nx), dtype=np.float64)
        x = np.arange(Nx//2)*dx + dx
        for i,t in enumerate(np.arange(Nt+1)*dt):
            theoretical_solution[i,sv_sample_indices] = np.sin(freq*np.pi*x/L) * np.exp(-np.power(freq*np.pi/L,2)*t*alpha)
        return theoretical_solution
...
def main():
    ...
    # for freq in range(5,9):
        ...
        # psi0 = np.random.uniform(-1.0, 1.0, 2**n) + np.random.uniform(-1.0, 1.0, 2**n)*1.0j
        if graycode_flag:
        ...
        for Di,D in enumerate(range(2,n+1)):
            if full_circle_flag:
                ...
                # if D < n:
                #     u_domains = [[j%n for j in range(i,i+D)] for i in range(n+1-D+1)]
                # else:
                #     u_domains = [list(range(n))]
                # match D:
                #     case 2:
                #         u_domains = [[0,1], [1,2], [2,3], [3,0]]
                #     case 3:
                #         u_domains = [[0,1,2], [2,3,0]]
                #     case 4:
                #         u_domains = [list(range(4))]
            else:
            ...
            fid_gs = np.zeros(times.shape[0], dtype=np.float64)
            psnr = np.zeros(times.shape[0], dtype=np.float64)
            mean_sq_err = np.zeros(times.shape[0], dtype=np.float64)
            mean_abs_err = np.zeros(times.shape[0], dtype=np.float64)
            ...
            # for i,t in enumerate(times):
                ...
                if graycode_flag:
                    ...
                    mean_sq_err[i] = np.mean((theoretical_solution[i,:] - (np.dot(iP_n, out.svs[i,:]).real * np.prod(out.c_list[0:i+1])))**2)
                    mean_abs_err[i] = np.mean(np.abs(theoretical_solution[i,:] - (np.dot(iP_n, out.svs[i,:]) * np.prod(out.c_list[0:i+1]))))
                else:
                    fid[i] = np.abs(np.vdot(theoretical_solution[i,:]/np.linalg.norm(theoretical_solution[i,:]), out.svs[i,:]))
                    mean_sq_err[i] = np.mean((theoretical_solution[i,:].real - (out.svs[i,:].real * np.prod(out.c_list[0:i+1])))**2)
                    psnr[i] = 10.0 * np.log10(np.max(theoretical_solution[i,:].real)**2/mean_sq_err[i])
                    mean_abs_err[i] = np.mean(np.abs(theoretical_solution[i,:] - (out.svs[i,:] * np.prod(out.c_list[0:i+1]))))
                fid_gs[i] = np.abs(np.vdot(gs, out.svs[i,:]))

            axs[Di,2].plot(times, fid)
            # axs[Di,2].plot(times,fid_gs,linestyle='--')
            # axs[Di,2].plot(times,psnr)
            axs[Di,3].plot(times, mean_sq_err)
            # axs[Di,3].plot(times, mean_abs_err)
        ...
        plt.tight_layout()
        plt.savefig(f'figs/heat_eqn/alpha/alpha={alpha:0.3f}_{n}_qubits_dtau={dtau:0.3f}_dx={dx:0.3f}_{"GrayCode" if graycode_flag else "SimpleEncoding"}_{"Linear" if not full_circle_flag else "Circular"}Topology_{"Full" if not reduce_dim_flag else "Reduced"}UnitaryOperatorSet_{"Homogeneous" if homogeneous_flag else "Cartesian"}Coordinates_Frequency={freq}.png')
